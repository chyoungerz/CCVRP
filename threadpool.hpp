...
namespace TP {
	/**
	 *  @brief std::thread::hardware_concurrency()类型的方便速记。应该评估为无符号int。
	 */
	...
	/**
	 *  @brief 一个辅助class，用于方便等待和/或同时获得多个future的结果。
	 *
	 * @tparam T future的返回类型
	 */
	template <typename T>
	class [[nodiscard]] multi_future {
	  public:
		/**
		 * @brief 用给定数量的future来构建一个multi_future对象
		 *
		 * @param num_futures_ 需要存储的future数量
		 */
		multi_future(const size_t num_futures_ = 0) : futures(num_futures_) {}

		/**
		 * @brief 从存储在这个multi_future对象中的所有future中获取结果，重新抛出任何存储的异常。
		 *
		 * @return 如果future返回void，这个函数也返回void。否则，它返回一个包含结果的vector。
		 */
		...
		/**
		 * @brief 获取存储在这个multi_future对象中的一个future的引用。
		 *
		 * @param i future的下标.
		 * @return future.
		 */
		...
		/**
		 * @brief 向这个multi_future对象追加一个future。
		 *
		 * @param future 追加的future。
		 */
		...
		/**
		 * @brief 获取存储在这个multi_future对象中的future数量。
		 *
		 * @return future数量
		 */
		...
		/**
		 * @brief 等待存储在这个multi_future对象中的所有future。
		 */
		void wait() const {
			for (size_t i = 0; i < futures.size(); ++i)
				futures[i].wait();
